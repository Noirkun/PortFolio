...
#include "GameModes/FarmWorldSettings.h"
#include "SaveGame/SaveSubsystem.h"
...
void APortFolioCharacter::SetEXP(double InEXP)
{

	int level=playerStatus.playerLevel;
	if(level!=maxLevel)
	{
		EXP += InEXP;
		if(EXP>=playerStatus.playerMaxEXP)
		{
			LevelUp();
		}
	...
	{		EXP += InEXP;
		if(EXP>=playerStatus.playerMaxEXP)
		{
			EXP=playerStatus.playerMaxEXP-1;
			UE_LOG(LogTemp,Display,TEXT("MAX経験値：%f,経験値：%f"),playerStatus.playerMaxEXP,EXP);
		}
	}
...
//#pragma region PlayerLevel関係
/**
 * @brief レベルアップ時
 */
void APortFolioCharacter::LevelUp()
{
	...
	if (StatusDataTable != nullptr)
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("Upper level has been reached"));
			return;
		}
	}
	else
